class SignificantFigures:
    ...
    def __init__(self, number):
        ...
        self.normalizedForm = None  # Nuevo atributo para la forma normalizada
        # Ejecutar el método 1 al crear el objeto
        self.checkDecimalSystem()
        # Ejecutar el método para calcular la forma normalizada
    ...
    # Método nuevo: Calcula la forma normalizada (notación científica) del número
    def calculateNormalizedForm(self):
        try:
            # Convertir a float para cálculos
            numFloat = float(self.number.strip())
            
            # Caso especial: cero
            if numFloat == 0:
                self.normalizedForm = "0.0 × 10^0"
                return
            
            # Determinar el exponente
            if abs(numFloat) >= 1:
                # Para números >= 1, contar cuántas posiciones mover el punto hacia la izquierda
                exponent = 0
                temp = abs(numFloat)
                while temp >= 10:
                    temp /= 10
                ...
                # Para números < 1, contar cuántas posiciones mover el punto hacia la derecha
                exponent = 0
                temp = abs(numFloat)
                while temp < 1:
                    temp *= 10
            ...
            # Formatear la mantisa con precisión adecuada
            if exponent == 0:
                self.normalizedForm = f"{mantissa}"
            else:
                self.normalizedForm = f"{mantissa:.6g} × 10^{exponent}"
                
        except ValueError:
            self.normalizedForm = "Forma normalizada inválida"
    
    # Método 3: Retorna un string 
    def toString(self):
        return f"Número: {self.number} Cifras Significativas: {self.significantFigures} Forma Normalizada: {self.normalizedForm}"
    ...
    def setNumber(self, number):
        ...
        # Recalcular cifras significativas y forma normalizada cuando se cambia el número
        self.checkDecimalSystem()
        self.calculateNormalizedForm()
